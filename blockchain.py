...
def mine_block():
    ...
    proof = proof_of_work()
    # Miners should be rewarded, so here is the reward transaction
    # reward_transaction = {
    #     "sender": "MINING",
    #     "recipient": owner,
...
def verify_chain():
    """ Verify the current blockchain and return True if valid, False if not"""
...
while waiting_for_input:
    ...
    if user_choice == "1":
        ...
        "previous_hash": "",
        "index": 0,
        "transactions": [{"sender": "Rachel", "recipient": "Alan", "amount": 1000.00}]
            }
    elif user_choice == "q":
        # This will lead to the loop to exit because it's running condition becomes False
        waiting_for_input = False
    else:
        print("\nInput", user_choice, "is invalid, please pick a value from the list!\n")
    if not verify_chain():
        print_blockchain_elements()
        print("Invalid blockchain")
        # Break out of the loop
        break
    print("\nThe balance of {}: {:6.2f}\n".format("Alan", get_balance("Alan")))
else:
    print("User has quit")
